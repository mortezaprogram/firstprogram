...
class StudentDB:
    headers = ['ID', 'First Name', 'Last Name', 'Street', 'City', 'State', 'Zip', 'Phone','Birth', 'Sex', 'Email', 'Lunch']
    student_info = []
    conn=0
    cursor=0
    query=0
    ...
    def setup_db(self):
        try:
            self.conn = mysql.connector.connect(host='localhost', database='students', user='studentadmin',password='P@ssw0rd')
    ...
    def update_table(self):
        for i in self.tree.get_children():
            self.tree.delete(i)
        self.query = "SELECT student_id, first_name, last_name, street, city, state, zip, phone, birth_date, sex, email, lunch_cost FROM student"
    ...
    def add_student(self):
        if (self.conn.is_connected() and not self.all_entries_filled(False)):
            ...
            self.query = f"INSERT INTO student VALUES( '{sid}', '{f_name}', '{l_name}','{street}', '{city}', '{state}', {zip}, '{phone}', '{birth}', '{sex}', '{format_date}', '{email}', {lunch})"
    ...
    def update_student(self):
        if (self.conn.is_connected() and not self.all_entries_filled(True)):
            ...
            sex = self.sex_entry_value.get()
            lunch = self.lunch_entry_value.get()
            self.query = f"UPDATE student SET first_name = '{f_name}', last_name = '{l_name}',email = '{email}', street = '{street}', city = '{city}', state = '{state}', zip = {zip}, phone = '{phone}',birth_date = '{birth}', sex = '{sex}', lunch_cost = {lunch} WHERE student_id = {sid}"
            self.execute_query(False)
            self.update_table()
    ...
    def delete_student(self):
        if (self.conn.is_connected()):
            if len(self.sid_entry_value.get()) == 0:
                ...
                sid = self.sid_entry_value.get()
                self.query = f"DELETE FROM student WHERE student_id={sid}"
                self.execute_query(False)
                self.update_table()
